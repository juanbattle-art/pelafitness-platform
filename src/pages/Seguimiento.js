...
// =============================================================================
// ESTILOS - Mantenemos identidad visual: negro + amarillo
// =============================================================================
const s = {
  ...
  // Tabs inferiores estilo MyFitnessPal
  ...
  // Círculo de calorías estilo MyFitnessPal
  ...
  // Momentos del día estilo MyFitnessPal
  ...
  // Vista detalle
  detail: { padding: '0' },
  detailCard: { background: '#111', border: '1px solid #222', borderRadius: 12, padding: 18, marginBottom: 14 },
...
// =============================================================================
// CIRCLE PROGRESS - Componente del círculo de calorías
// =============================================================================
...
export default function Seguimiento({ perfil }) {
  ...
  const [tab, setTab] = useState('panel') // panel, diario, progreso, mas
  ...
  const [searchTab, setSearchTab] = useState('todo') // todo, recientes
  ...
  async function cargarTodo() {
    const id = perfil.id
    const [{ data: p }, { data: a }, { data: c }, { data: e }, { data: al }, { data: m }, { data: rec }] = await Promise.all([
      ...
      supabase.from('alimentos').select('*').order('nombre').limit(500),
  ...
  function buscarAlimento(q) {
    ...
    timeoutRef.current = setTimeout(async () => {
      setBuscando(true)
      try {
        // Búsqueda Argentina/Uruguay primero
        const urlLocal = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&tagtype_0=countries&tag_contains_0=contains&tag_0=argentina&search_simple=1&action=process&json=1&page_size=20&lc=es&sort_by=unique_scans_n`
        ...
        if (dataLocal.products && dataLocal.products.length > 0) {
          resAPI = procesarProductosOFF(dataLocal.products, codigosVistos)
        }

        // Si hay menos de 8 resultados, ampliar a global
        if (resAPI.length < 8) {
          const urlGlobal = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&search_simple=1&action=process&json=1&page_size=25&lc=es&sort_by=unique_scans_n`
          const responseGlobal = await fetch(urlGlobal)
          const dataGlobal = await responseGlobal.json()
          if (dataGlobal.products) {
            const masResultados = procesarProductosOFF(dataGlobal.products, codigosVistos)
            resAPI = [...resAPI, ...masResultados]
          }
        }

        setResultados([...resLocales, ...resAPI.slice(0, 25)])
      } catch (err) {
        console.error('Error OFF:', err)
      } finally {
        setBuscando(false)
      }
    }, 400)
  ...
  function procesarProductosOFF(productos, codigosVistos) {
    return productos
      .filter(p => {
  ...
  function calcularGramosFinal() {
    ...
    // Si es una porción del producto
  ...
  async function agregarComida() {
    ...
    try {
      ...
      }, { onConflict: 'alumno_id,alimento_data' })
    } catch(e) { console.log('No se pudo guardar reciente:', e) }
  ...
  // Lista de resultados según tab del buscador
        ...
        {tab === 'mas' && (
          <div>
            ...
            <div style={s.card}>
              ...
              <div style={{ fontSize: 11, color: '#444', marginTop: 12, fontStyle: 'italic' }}>Editar metas: por ahora desde la versión anterior. Próximamente desde acá.</div>
            </div>
          </div>
        )}
      </main>
      ...
      {showSearch && (
        <div style={s.searchModal}>
          ...
          <div style={s.searchResults}>
            {buscando && <div style={s.loader}>🔍 Buscando en millones de alimentos...</div>}
            {!buscando && searchTab === 'todo' && busqueda.length < 2 && <div style={s.empty}>Escribí al menos 2 letras para buscar.<br/><br/>Tip: probá "Yogurísimo", "Pepitos", "Conaprole"...</div>}
            ...
            {resultadosVisibles.map(a => (
              <div key={a.id} style={s.resultItem} onClick={() => seleccionarAlimento(a)}>
                <div style={s.resultImg}>
                  {a.imagen_url ? <img src={a.imagen_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 8 }} onError={e => { e.target.style.display='none'; e.target.parentNode.innerHTML='🍽️' }} /> : '🍽️'}
                </div>
                <div style={s.resultInfo}>
                  ...
                  <div style={s.resultMeta}>
                    ...
                    {a.calorias} kcal · {a.proteinas}g prot · {a.carbohidratos}g carb · {a.grasas}g gras (100g)
                  </div>
                </div>
              </div>
            ))}
